...
if (isset($_POST['passwordreset'], $_POST['passwordconf'], $_POST['reset_token_hash'], $_POST['user_email'])) {
    ...
    $password =  validate($_POST['passwordreset']);
    $re_pass = validate($_POST['passwordconf']);

    //$hashedpassword = password_hash($password, PASSWORD_DEFAULT);

    $hashedpassword = md5($password);

    $loginpage = 'http://localhost/beginers/Login%20training/Log-in-No-talking/Modern-Login-master%20-%20Reset-pass%20-%20ERRORS/index.php';
    $reset =  " http://localhost/beginers/Login%20training/Log-in-No-talking/Modern-Login-master%20-%20Reset-pass%20-%20ERRORS/forgot-password/reset-password.php?key=" . $emailId . "&token=" . $token . " ";
    ...
    if ($password !== $re_pass) {
        //The confirmation password does not match
          echo "<h1 style='text-align: center; color:red; height: 100%; width: 100%; display: flex; position: fixed; align-items: center; justify-content: center;'><a href='$reset' style='color:red;'>Паролата не се съвпада</a></h1>";
          exit();
      }
